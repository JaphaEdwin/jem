...
/* ══════════════════════════════════════════════════════════════
   ...
   Admin-integrated: reads live content from persistent storage
   ══════════════════════════════════════════════════════════════ */
...
/* ─── APPLY CMS CONFIG — overwrites defaults with admin-saved values ─── */
function applyCmsConfig(cfg) {
  if (!cfg) return;
  if (cfg.site) SITE = { ...SITE, ...cfg.site };
  if (cfg.fees) FEES = cfg.fees;
  if (cfg.startTimes) START_TIMES = cfg.startTimes;
  if (cfg.pillars) PILLARS = cfg.pillars;
  if (cfg.instruments) INSTRUMENTS = cfg.instruments;
  if (cfg.sponsorTiers) SPONSOR_TIERS = cfg.sponsorTiers;
  if (cfg.itineraries) ITINERARIES = cfg.itineraries;
  if (cfg.faqs) FAQS = cfg.faqs;
  if (cfg.courseMarkers) COURSE_MARKERS = cfg.courseMarkers;
  if (cfg.aidStations) {
    AID_STATIONS = cfg.aidStations.map(s =>
      Array.isArray(s) ? s : [s.name, s.location, s.water, s.electrolytes, s.medical]
    );
  }
  if (cfg.raceWeekSchedule) RACE_WEEK_SCHEDULE = cfg.raceWeekSchedule;
  if (cfg.seo) SEO = { ...SEO, ...cfg.seo };
}
...
function AppMain() {
  ...
  /* Load CMS config from persistent storage on mount */
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get(CMS_STORAGE_KEY);
        if (result) {
          const cfg = JSON.parse(result.value);
          applyCmsConfig(cfg);
          setConfigVersion(v => v + 1); /* force re-render with new data */
        }
      } catch (e) {
        /* No saved config — use defaults silently */
      }
      setCmsReady(true);
    })();
  }, []);

  /* Loading splash while CMS loads */
  if (!cmsReady) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: P.bg, fontFamily: "var(--ff-body)" }}>
